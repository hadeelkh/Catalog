...
from flask import session as login_session, jsonify, flash
...
def fbconnect():
    ...
    token exchange we have to split the token
    ...
    """Token must be stored in login_session to properly logout"""
...
def CategoryItemJSON(category_id):
    Items = session.query(
        CatItems).filter_by(category_id=category_id).all()
...
def showCategories():
    ...
    lastItems = session.query(
        CatItems.id, CatItems.title, Categories.name).order_by(
        desc(CatItems.id)).join(CatItems.categories).limit(6)
    return render_template(
     'categories.html', categories=categories,
     lastItems=lastItems)


# Show all categories and specific category items
@app.route('/Categories/<string:categoryName>/items')
def showCatItems(categoryName):
    categories = session.query(Categories)
    category = session.query(
        Categories).filter_by(name=categoryName).one()
    AllItems = session.query(
        CatItems).filter_by(category_id=category.id).all()
    itemsCount = len(AllItems)
    return render_template(
        'categoryItem.html', categories=categories,
        AllItems=AllItems, category=category, itemsCount=itemsCount)


# show item details
@app.route('/Categories/<string:categoryName>/<int:Item>')
def showItemDetails(categoryName, Item):
    ItemDet = session.query(CatItems).filter_by(id=Item).one()
    return render_template(
            'ItemDetails.html', Item=ItemDet, categoryName=categoryName)
...
def newItem():
    ...
    if request.method == 'POST':
        newitem = CatItems(
            title=request.form['title'], description=request.form[
...
# edit item
@app.route('/Categories/<string:categoryName>/<int:Item>/edit/', methods=[
        'GET', 'POST'])
@login_required
def editItem(categoryName, Item):
    editedItem = session.query(CatItems).filter_by(id=Item).one()
    categories = session.query(Categories)
    ...
    if request.method == 'POST':
        ...
        return redirect(
            url_for(
                'showItemDetails', categoryName=categoryName,
                Item=editedItem.id))
    else:
        return render_template(
            'editItem.html', Item=editedItem, categoryName=categoryName,
            categories=categories)


# Delete an item
@app.route('/Categories/<int:Item>/delete/', methods=['GET', 'POST'])
@login_required
def deleteItem(Item):
    deleteItem = session.query(CatItems).filter_by(id=Item).one()
